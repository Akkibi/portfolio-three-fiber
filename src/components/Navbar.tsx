...
const Navbar: React.FC<NavbarProps> = ({ progress, isMobile, isHome }) => {
  ...
  return (
    <nav ref={navRef} className="fixed z-10 w-full h-full pointer-events-none ">
      ...
      >
        {ProjectsData.map((_, index) => (
          <div
            ...
            style={{
              height: `${progressNavScale(progress - index * 2)}px`,
            }}
          ></div>
        ))}
      </div>
      <div className=" absolute p-5 flex flex-col gap-5">
        <Link className="pointer-events-auto text-3xl" tabIndex={0} to="/">
          AKIRA VALADE
        </Link>
        <p
          className=" max-w-72"
      ...
      <div className="flex flex-col place-items-end gap-4 absolute bottom-0 right-0 p-5">
